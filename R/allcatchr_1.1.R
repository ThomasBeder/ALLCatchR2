 ...
 cat("Blast count prediction...\n")
  ...
  if (Lineage == "T-ALL") {
   ...
   if(is.null(Counts.file)){
    ...
    length(TALL_subtype_model_list)
    
    
    TALL_subtype_peds <- list()
    
    for (y in 1:length(TALL_subtype_model_list)) {
    TALL_model <- TALL_subtype_model_list[[y]]
    ...
    for (x in 1:length(TALL_model)) {
      model_type <- modelType[x]
      if (model_type == "determenistic") {
        preds <- list()
        for (j in 1:length(TALL_model[[x]])) {
          preds[[j]] <- as.character(predict(TALL_model[[x]][[j]], newdata = Counts.norm,type = "raw"))
        }
        preds <- as.data.frame(do.call("cbind", preds))
        head(preds)
        
        preds_prob <- data.frame(matrix(0, nrow = nrow( Counts.norm),
                                        ncol = length(c(TALL_model$model_svmLinear[[1]]$levels[1],
                                                        TALL_model$model_svmLinear[[1]]$levels[2]))
        ))
        colnames(preds_prob) <- c(TALL_model$model_svmLinear[[1]]$levels[1],
                                  TALL_model$model_svmLinear[[1]]$levels[2])               
        
        for (i in 1:nrow(preds)) {
          for (j in 1:ncol(preds_prob)) {
            preds_prob[i,j] <- length(which(preds[i,] == make.names(colnames(preds_prob)[j])))/length(TALL_model[[1]])
      ...
      if (model_type == "probalistic") {
        preds_prob <- list()
        for (j in 1:length(TALL_model[[x]])) {
          preds_prob[[j]] <- predict(TALL_model[[x]][[j]], newdata =Counts.norm,type = "prob")
        }
        tail(preds_prob[[j]])
        ...
        head(preds_prob)
        tail(preds_prob)
        colnames(preds_prob) <- sort(make.names(c(TALL_model$model_svmLinear[[1]]$levels[1],
                                                  TALL_model$model_svmLinear[[1]]$levels[2])))
      } 
    ...
    colnames(integrated_model) <- make.names(c(TALL_model$model_svmLinear[[1]]$levels[1],
                                               TALL_model$model_svmLinear[[1]]$levels[2]))
    integrated_model <- data.frame(sample = rownames(Counts.norm),
     ...
     TALL_subtype_peds[[y]] <- integrated_model[,2]
    }  
    # combine predictions from the individual T-ALL subtype models
    TALL_subtype_peds <- as.data.frame(do.call("cbind", TALL_subtype_peds))
    rownames(TALL_subtype_peds) <- rownames(Counts.norm)
    colnames(TALL_subtype_peds) <- names(TALL_subtype_model_list)
    ...
    # assign subtype predictions based on cutoffs ##############################
    TALL_subtype_cutoffs <- TALL_subtype_cutoffs[match(colnames(TALL_subtype_peds), TALL_subtype_cutoffs$TALLsubtype), ]
    TALL_subtype_peds$`T-ALL main-cluster high-confidence` <- NA
    TALL_subtype_peds$`T-ALL sub-cluster high-confidence` <- NA
    TALL_subtype_peds$`T-ALL immature high-confidence` <- NA
    TALL_subtype_peds$`T-ALL main-cluster candidate` <- NA
    TALL_subtype_peds$`T-ALL sub-cluster candidate` <- NA
    TALL_subtype_peds$`T-ALL immature candidate` <- NA
    
    # make predictions
    for (i in 1:nrow(TALL_subtype_peds)) {
      # main cluster predictions high confidence
      pos <- which(TALL_subtype_peds[i, 1:17] >= TALL_subtype_cutoffs$`high-confidence`[1:17])
      TALL_subtype_peds$`T-ALL main-cluster high-confidence`[i] <- paste0(colnames(TALL_subtype_peds)[pos], collapse = ";")
      
      # sub cluster predictions high confidence
      pos <- which(TALL_subtype_peds[i, 18:22] >= TALL_subtype_cutoffs$`high-confidence`[18:22])
      TALL_subtype_peds$`T-ALL sub-cluster high-confidence`[i] <- paste0(colnames(TALL_subtype_peds)[18:22][pos], collapse = ";")
      
      # immature T-ALL / ETP-like high confidence
      pos <- which(TALL_subtype_peds[i, 23] >= TALL_subtype_cutoffs$`high-confidence`[23])
      TALL_subtype_peds$`T-ALL immature high-confidence`[i] <- paste0(colnames(TALL_subtype_peds)[23][pos], collapse = ";")
      
      
      # main cluster predictions candidate
      pos <- which(TALL_subtype_peds[i, 1:17] >= TALL_subtype_cutoffs$`candidate`[1:17])
      TALL_subtype_peds$`T-ALL main-cluster candidate`[i] <- paste0(colnames(TALL_subtype_peds)[pos], collapse = ";")
      
      # sub cluster predictions candidate
      pos <- which(TALL_subtype_peds[i, 18:22] >= TALL_subtype_cutoffs$`candidate`[18:22])
      TALL_subtype_peds$`T-ALL sub-cluster candidate`[i] <- paste0(colnames(TALL_subtype_peds)[18:22][pos], collapse = ";")
      
      # immature T-ALL / ETP-like candidate
      pos <- which(TALL_subtype_peds[i, 23] >= TALL_subtype_cutoffs$`candidate`[23])
      TALL_subtype_peds$`T-ALL immature candidate`[i] <- paste0(colnames(TALL_subtype_peds)[23][pos], collapse = ";")
    ...
    ## set subcluster scores to NA in cases not predicted to be C1
    ...
    TALL_subtype_peds$`T-ALL main-cluster candidate` <- sub("^;", "", TALL_subtype_peds$`T-ALL main-cluster candidate`)
    TALL_subtype_peds$`T-ALL main-cluster high-confidence` <- sub("^;", "", TALL_subtype_peds$`T-ALL main-cluster high-confidence`)
    TALL_subtype_peds$`T-ALL main-cluster candidate` <- sub(";$", "", TALL_subtype_peds$`T-ALL main-cluster candidate`)
    TALL_subtype_peds$`T-ALL main-cluster high-confidence` <- sub(";$", "", TALL_subtype_peds$`T-ALL main-cluster high-confidence`)


    TALL_subtype_peds$`T-ALL immature candidate`[TALL_subtype_peds$`T-ALL immature candidate` == 
...
for (l in 1:nrow(TALL_subtype_cutoffs)) {
  plot <-  plot + ggplot2::annotate("segment", y = l-0.5, yend = l+0.5, x = TALL_subtype_cutoffs$candidate[l], xend =  TALL_subtype_cutoffs$candidate[l], colour = "black", size = 0.5, alpha = 0.5)
}
